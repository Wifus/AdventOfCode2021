...
for (const draw of draws) {
    ...
    if (boards.some((_, i) => checkBoard(_) && (boards.length > 1 ? boards.splice(i, 1) && false : lastdraw = draw))) break;
}

const sum = boards[0].reduce((acc, cur) => acc += cur.reduce((acc, cur) => acc += cur ?? 0), 0);
